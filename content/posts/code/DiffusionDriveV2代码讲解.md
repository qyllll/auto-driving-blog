...
官方仓库 `hustvl/DiffusionDriveV2` 已开源（`https://github.com/hustvl/DiffusionDriveV2`，MIT 协议，345+ stars）。论文给出了完整的 Algorithm 描述（含 reward 定义、advantage 计算、loss 公式），以下目录结构基于实际仓库 + 论文补充的 Algorithm 还原。标注 `(new)` 的文件是相对 DiffusionDrive 新增的，标注 `(mod)` 是修改的。
