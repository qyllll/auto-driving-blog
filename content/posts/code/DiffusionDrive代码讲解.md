...
下面用纯 ASCII 的树来展示目录结构（注意：所有层级都用 `--` 和 `|` 这种等宽字符，绝不用 Unicode 框线符号，避免等宽字体下错位）：

```
navsim/                                    # <- fork 自 autonomousvision/navsim（官方管线，内联）
|-- agents/diffusiondrive/                 # ★ 作者新增的模型代码（挂在 navsim 上的"插件"）
|   |-- transfuser_agent.py                # Agent 包装类（继承 AbstractAgent，负责 build 特征/调用模型/输出轨迹）
|   |-- transfuser_model_v2.py             # ★ 主模型 V2TransfuserModel + 截断扩散策略 TrajectoryHead
|   |-- transfuser_backbone.py             # 图像/LiDAR 主干（TransFuser 融合 backbone 的实现）
|   |-- transfuser_features.py             # 特征/目标构建（环视三相机拼接、BEV、ego/map token 在这）
|   |-- transfuser_loss.py                 # 顶层损失聚合（把各项 loss 加起来）
|   |-- transfuser_config.py               # 全部超参数（网络结构、anchor 路径、权重等）
|   `-- modules/                           # 模型内部的小模块子目录
|       |-- multimodal_loss.py             # ★ 多模态 cls+reg 损失（找最近 anchor 当正样本）
|       |-- conditional_unet1d.py          # 条件去噪网络（论文里用的 1D UNet 变体，本项目实际用 transformer 版）
|       `-- scheduler.py                   # 扩散调度器封装（DDIM 的时间步逻辑）
...
l0 = cameras.cam_l0.image[28:-28, 416:-416]   # 左相机，裁掉上下和左右多余部分
f0 = cameras.cam_f0.image[28:-28]             # 前相机，只裁上下
r0 = cameras.cam_r0.image[28:-28, 416:-416]   # 右相机，裁掉上下和左右多余部分
# stitch l0, f0, r0 images
stitched_image = np.concatenate([l0, f0, r0], axis=1)   # 沿宽度方向横拼
resized_image = cv2.resize(stitched_image, (1024, 256)) # 统一缩放到 1024x256 全景
...
plan_anchor = np.load(plan_anchor_path)          # 从文件读入 20 条模板轨迹
self.plan_anchor = nn.Parameter(
    torch.tensor(plan_anchor, dtype=torch.float32), requires_grad=False,
)  # 形状 [20, 8, 2]：20 个 anchor，每条 8 个点，每个点只有 x,y（朝向后面再补）
```
...
odo_info_fut = self.norm_odo(plan_anchor)                         # 归一化到合理范围
timesteps = torch.randint(0, 50, (bs,), device=device)           # 截断到 50（不是 1000）
noise = torch.randn(odo_info_fut.shape, device=device)            # 随机高斯噪声
noisy_traj_points = self.diffusion_scheduler.add_noise(
    original_samples=odo_info_fut, noise=noise, timesteps=timesteps,
).float()
noisy_traj_points = torch.clamp(noisy_traj_points, min=-1, max=1) # 限制范围
noisy_traj_points = self.denorm_odo(noisy_traj_points)            # 反归一化
```
...
plan_reg = traj_delta.reshape(bs, ego_fut_mode, self.ego_fut_ts, 3)  # [B,20,8,3]
```
...
mode_idx = poses_cls.argmax(dim=-1)                              # 找分数最高的模式
mode_idx = mode_idx[..., None, None, None].repeat(1, 1, self._num_poses, 3)
best_reg = torch.gather(poses_reg, 1, mode_idx).squeeze(1)       # 取出该模式的轨迹
```
...
为了清晰，我们用纯 ASCII 的缩进树展示这 7~8 步（绝不使用任何 Unicode 框线字符，全部用 `--` 和 `|` 这种等宽 ASCII）：
...
img      = stitch_3_cameras(agent_input.cameras)   # [B, 3, 256, 1024]
lidar_bev = project_lidar_to_bev(agent_input.lidar) # [B, C, H, W]
ego_token, map_token = encode_ego_map(agent_input)  # 若干向量
...
```
img_feat  = ResNet34(img)        # 图像过 ResNet，得到图像特征 [B, d, h, w]
bev_feat  = ResNet34(lidar_bev)  # 激光雷达 BEV 过另一个 ResNet [B, d, h', w']
# 两者做交叉注意力融合：图像特征去 BEV 里找对应、BEV 去图像里找对应
fused_feat = cross_attention(img_feat, bev_feat)   # [B, N_token, d]
# 再把 ego/map token 也融进去
fused_feat = fused_feat + ego_token + map_token
输出: fused_feat  # 统一的"场景理解"特征，后面去噪要用
```
...
```
plan_anchor = self.plan_anchor                    # [20, 8, 2]，冻结的模板轨迹
anchor = plan_anchor.unsqueeze(0).repeat(B,1,1,1) # 复制到 batch：[B, 20, 8, 2]
anchor = norm_odo(anchor)                          # 归一化到 [-1,1] 附近
输出: anchor_embed = anchor   # [B, 20, 8, 2]，20 个模式的初始草稿
```
...
```
noise = randn_like(anchor_embed)                  # 随机噪声 [B, 20, 8, 2]
t = 8  (固定截断时间步，对所有样本一样)
noisy = diffusion_scheduler.add_noise(anchor_embed, noise, t)  # [B, 20, 8, 2]
输出: noisy  # 轻微模糊的 20 条草稿
```
...
```
current = noisy                                  # [B, 20, 8, 2]
for step in range(2):                            # 只循环 2 次
    # 用场景特征 + 当前轨迹，预测"该往哪修"
    offset = denoiser(
        traj=current,                            # 当前（模糊）轨迹 [B,20,8,2]
        scene=fused_feat,                        # 场景特征 [B, N_token, d]
        t=current_timestep,                      # 当前去噪步数，作 FiLM 调制
    )                                            # offset: [B, 20, 8, 2]
    current = current + offset                   # 残差回归：轨迹 = 旧轨迹 + 修正量
# 最后补上朝向角 theta，凑成 [B, 20, 8, 3]
plan_reg = concat([current, theta_head(current)], dim=-1)  # [B, 20, 8, 3]
输出: plan_reg  # 20 条已经雕好的具体轨迹
```
...
```
plan_cls = cls_head(plan_reg, fused_feat)        # [B, 20]，每条模式一个置信度
输出: plan_cls  # 20 个分数，越大表示模型越认为这条靠谱
```
...
```
mode_idx = plan_cls.argmax(dim=-1)               # [B]，每个场景选分数最高的模式下标
best_reg = plan_reg.gather(1, mode_idx)          # 取出该模式的轨迹 [B, 8, 3]
输出: best_reg  # 最终选中的 1 条最优轨迹
```
...
```
# 准备起点
anchor = plan_anchor.repeat(B,1,1,1)              # [B,20,8,2]
x = add_noise(anchor, noise, t=8)                 # 截断起点，只加一点点噪声
# DDIM 时间步序列：从接近 8 的位置往 0 走 2 步
timesteps = [8, 4]   (示意，实际由 step_ratio 算)
for t in timesteps:
    noise_pred = denoiser(x, fused_feat, t)       # 预测噪声/offset
    x = ddim_step(x, noise_pred, t, t_prev)       # 一步去噪更新
# 得到清晰轨迹后补 theta，再 argmax 选 1 条
plan_reg = concat([x, theta_head(x)], -1)         # [B,20,8,3]
best = plan_reg.gather(1, plan_cls.argmax(-1))    # [B,8,3]
```
