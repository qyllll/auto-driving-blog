...
│       ├── sparsedrive_agent.py      # Agent（继承 AbstractAgent）★入口
│       ├── sparsedrive_model.py      # 模型定义 + 加载冻结词汇表 ★
│       ├── sparsedrive_backbone.py   # ResNet-34 主干
│       ├── custom_decoder.py         # ★ 两级评分 + 组合 + 损失（核心）
│       └── scorer/get_pdm_score_v2.py# PDM 打分（复用 navsim 仿真器）
...
```
scripts/cluster/cluster_anchor.py    # 离线 KMeans 建 path/velocity 词表 → .npz
scripts/training/sparsedrive_navsimv1.sh  # 只是个调 run_training.py 的启动脚本
```
...
└─> ③ SparseDriveModel.forward(features)
     ...
     └─ ⑦ CustomTransformerDecoder(              # 2 层，这是"打分器"
            ...
            │     · 此刻候选 = 64 × 20 = 1280 条组合轨迹
            │
            ├─ 第 1 层 :
            │     · 再各 top-k 一次（配置 path_filter_num/velocity_filter_num）
            │     · 把筛后的 path_embed ⊕ vel_embed 逐元素相加 → 组合轨迹嵌入
