...
- **怎么算（重叠检测的具体做法）**：在 4 秒仿真的每一帧，自车和每个其他交通参与者（车、人、骑手）都有一个**俯视（BEV）下的有向包围盒（Oriented Bounding Box, OBB）**——由自身位置 $(x,y)$、朝向 $\theta$、以及长宽尺寸（车约 $4.7\text{m} \times 1.8\text{m}$，行人更小）定义的一个矩形。碰撞检测就是判断**两个 OBB 是否相交**：
  1. 实际用的是**略微膨胀**的包围盒（通常四周加一个几厘米到十几厘米的安全余量），让判定更保守、抗定位噪声；
  2. 相交判定靠**分离轴定理（SAT, Separating Axis Theorem）**：把两个矩形各自投影到 4 条候选轴（两个盒子的各两条边法线）上，只要**任意一条轴上两个投影区间不重叠**，就说明两盒分离、没撞；**4 条轴全部重叠**才判定为碰撞；
  3. 只要某一帧、对某一个参与者算出"相交" → 记一次碰撞，再走责任判定。
  - **自车有责**（ego 主动侵入别人路径、留的 gap 不够等）→ NC = **0**
  - **自车无责**（被后车追尾、对方偏离日志撞上来等）→ NC = **0.5**
  - **全程没碰撞** → NC = **1**
- **责任怎么定**：判出重叠后，NAVSIM 会给这次碰撞贴"责任标签"——核心逻辑是看**谁动到了别人的地盘**：如果自车在运动、且是它主动进入/挤压了另一个本就在那儿的参与者的盒子 → 自车有责（0）；如果是别的参与者偏离了自己的日志轨迹、主动撞上自车（典型如自车停着被追尾）→ 自车无责（0.5）。所以 NC 的 0.5 不是"打了折的撞"，而是"确实撞了，但锅不在你"。
...
- **怎么算**：逐帧检查自车中心是否落在"可行驶区域"多边形内（含一小段扩展余量，通常是车道 + 应急车道）。**只要任何一帧越界（骑路沿、上人行道、越野）→ DAC = 0；全程都在 → DAC = 1**。判断"点是否在多边形内"用的是标准的射线法（ray casting）：从自车中心引一条射线，数与多边形边界的交点数，奇数在内部、偶数在外部。
- **关键认知：你问的"撞出去才会扣分"——对的，但要理解"撞出去"在这指的是什么**。DAC 扣分的唯一条件是"自车位置离开了可行驶区域"，而**和"有没有发生碰撞"是两件独立的事**：
  - 你在机动车道上正常开、却追尾前车 → 你整条轨迹都在车道里 → **DAC 仍是 1**，碰撞只影响 NC/TTC，不影响 DAC；
  - 只有当你**规划的路径本身就冲出了路面**（比如为躲车向左猛打、整个轨迹画到了人行道上，或者在路口外越野）→ 这时候 DAC 才掉到 0。
  - 一个常被忽略的点：**非反应式仿真里，自车并不会被"物理撞飞"**。背景车按日志走、互不反应，自车用 LQR 跟踪**自己规划的整条 4 秒轨迹**——发生碰撞只是"检测到盒子重叠"，自车并不会真的被顶出路外。所以 DAC 完全取决于**你规划的路径几何在不在路上**，跟"撞没撞"无关。也就是说：DAC 掉 0 一定是"路径自己画出了路面"，而未必伴随碰撞；反过来，路上撞车也不会顺带把 DAC 拉低。

#### 3. TTC（碰撞时间）—— 看"最小安全时距"有没有跌破阈值

- **怎么算（每帧怎么求"还有多久会撞"）**：在轨迹的每一帧，取自车**当前的状态（位置 + 速度/航向）**和每个其他参与者**当前的状态**，假设**双方都保持这一刻的运动不变**（匀速、不转向、不刹车），去推算"两个盒子多久后第一次重叠"：
  1. 把问题变成**相对运动**：相对速度 $\vec{v}_{\text{rel}} = \vec{v}_{\text{ego}} - \vec{v}_{\text{other}}$，相对位置就是两盒心连线；
  2. 向前以很小的时间步 $\Delta t$ 推演（或解析求解）：自车盒子按 $\vec{v}_{\text{ego}}$ 平移、对方盒子按 $\vec{v}_{\text{other}}$ 平移，用和 NC 一样的 OBB 相交判定（SAT）检查这一刻是否重叠；
  3. 第一个出现重叠的时刻 $\tau$ 就是这对参与者的 **TTC**；对所有参与者取最小，得到这一帧的 TTC；
  4. 再对**整条轨迹所有帧**的 TTC 取最小值，就是该场景的 TTC。若这个全局最小值 **< 临界阈值（配置项 `critical_ttc`，默认一个很小的秒数）→ TTC = 0**；否则 → **1**。
  - 直觉：它就是"如果大家都维持现状不变，最早几秒后会亲上"——近距 + 高速对开 → TTC 很小；远或同向同速 → TTC 很大（甚至判为不冲突）。
...
> **EP 低，到底是"开太慢"还是"撞车"导致的？——答案：几乎总是"开太慢/路径太短"，和撞车无关。** 这一点最容易混，单独讲清：
> - **EP 只看你规划的路径"沿路线走了多远"，不关心有没有撞**。而且非反应式仿真里自车会**走完自己规划的整条 4 秒轨迹**（LQR 跟踪，碰撞只是检测到、不会让车停下或缩路径）——所以哪怕你在第 1 秒就撞了，后面 3 秒的轨迹仍然照走，EP 按这条完整路径算。
> - 因此：**低 EP 的本质是"这条 4 秒轨迹本身覆盖的路程短"**，而路程短通常是因为**速度开得慢**（或模型输出了一条保守、短促的轨迹）。它和是否发生碰撞没有因果关系。
> - 反例最能说明问题：一条**开得飞快、但中途撞了**的轨迹，EP 反而可能**很高**（它 4 秒里真跑出去很远），只是会被 NC=0 / TTC=0 拖累；一条**慢慢吞吞、全程没撞**的轨迹，EP **很低**（路程短），但 NC/DAC 满分。所以"我撞了所以 EP 低"是个错觉——你看到的同时低 EP + 撞车，往往是因为那条轨迹既慢又撞，但 EP 低是"慢"贡献的，不是"撞"贡献的。
> - 一句话：**EP 低 = 路径没铺够远 = 开太慢（或规划太保守）；撞车影响的是 NC 和 TTC，动不了 EP。**

> **疑问：那 NAVSIM 是不是"不真实"？真实世界里第 1 秒撞了，后面就没有轨迹、也不可能有 EP 了，可 NAVSIM 还会跑完剩下 3 秒——对吗？**
> **A：对的，这正是它的设计。** NAVSIM 跑的是**非反应式仿真（non-reactive simulation）**：模型提交的是一条预先算好的、固定长度的 4 秒轨迹（一串 $(x,y,\text{heading})$ 点），仿真时
> 1. 自车由一个 **LQR 控制器去跟踪这条固定轨迹**——只负责把车摆到每个时刻该在的位置；
> 2. 背景车按日志里的真实轨迹走，且**互不反应**（不会因为你来了就刹车/让）；
> 3. 仿真器**没有任何碰撞物理**——不计算动量、不把车弹开、不让你"撞停"。它每帧只干两件事：把车摆到轨迹指定的位置 + 检查违规（盒子重叠=碰撞、越界=DAC 等）。
>
> 所以"第 1 秒撞了"在 NAVSIM 里**只是"第 1 秒那一帧检测到盒子重叠"**，轨迹本身不会被截断或改写，后面 3 秒该摆哪还摆哪，EP 按这条**完整 4 秒路径**算进度——跟真实世界"撞了就失能、后面没轨迹"完全不同。
>
> **为什么这么设计（而不是像真实世界撞了就停）：**
> - **可复现 + 便宜**：不需要物理引擎，所有方法在同一套确定性脚本下比，分数才可比；
> - **PDMS 评的是"这条规划轨迹本身安不安全/合不合规"**，不是"撞了之后会发生什么"——后者是 CARLA 那种闭环仿真的事，NAVSIM 故意不做闭环；
> - **这也是个已知局限**：真实世界你撞了就完了，NAVSIM 里一条早期就撞的轨迹仍会因"整条路径覆盖了距离"拿到一个 EP（甚至不低），只是被 **NC=0 乘性归零**掩盖了。所以 PDMS 对"早期碰撞"的敏感度全在 NC，不体现在 EP 上。
>
> 一句话：**NAVSIM 的 EP 永远基于"模型规划出的完整 4 秒路径"算，与路上是否真撞无关——因为仿真器根本不模拟撞车后的物理后果，轨迹是固定的、跑完的。**
...
| 前进进度 | EP | 加权 | 5 | $[0,1]$ | **v2 升级**：由单专家改为多路径进度 MPP |
...
| 维度 | v1（PDMS） | v2（EPDMS） |
|:----|:----|:----|
| 乘性惩罚项 | NC、DAC | NC、DAC、**DDC（启用，v1 里权重=0）**、**TLC（新增）** |
| 加权鼓励项 | TTC(5)、EP(5)、C(2) | TTC(5)、EP(5，**MPP 升级**)、**LK(2,新增)**、**HC(2,新增)**、**EC(2,新增)** |
| EP 的参考轨迹 | 单条人类专家轨迹 | **多路径进度 MPP**：多条候选参考路径取最优，避免"走别的合理车道"被错罚 |
| 人类过滤 | 无 | **filter$_m$**：同一场景人类专家也违规则该项免罚（防误伤合理绕行） |
| 背景车交互 | 非反应式（按日志轨迹走、互不反应） | 背景车改用 **IDM 规则模型**做反应式交互 |
| 时序处理 | 孤立的 4 秒预测片段、离散帧采样 | **HC 前接 1.5s 历史 + EC 查相邻帧跳变** → 引入"连续时序一致性" |
| 总风格 | "大致安全合规" | "精细安全合规 + 时序自洽" |

一句话：**v2 = v1 的框架 + 把 v1 里躺平的 DDC 激活 + 新增 TLC/LK/HC/EC 四个精细项 + EP 改成多路径 + 加入人类过滤与反应式背景车 + 把时序一致性补上**。所以同一方法在 navtest（PDMS）和 navtest v2（EPDMS）上分数会差一截——榜单上 EPDMS 普遍比 PDMS 低几个点，不是模型变差了，是尺子更严了。

#### 每个 v2 子指标到底怎么算（详解）

> NC / DAC / TTC 的计算与 v1 完全一致（见上一节 bounding-box 重叠、射线法、相对运动首次重叠时刻），这里只讲 v2 **新增或改动**的子指标。

**① DDC（行驶方向合规 / Driving Direction Compliance）——v1 躺平、v2 启用**
- **怎么算**：沿**路线（route）方向**检查自车是否在"朝目标前进"，而不是原地打转、倒车刷安全分。具体是把自车进度投影到路线切向：若前进方向的投影为正且大致沿预期走向 → DDC=1；若明显逆行 / 绕圈 / 侧向偏移过大 → 按严重程度给 0 或 0.5（三档 $\{0, \tfrac12, 1\}$，责任判定逻辑同 NC 的"有责/无责"）。它和 EP 的区别在于：EP 只关心"走了多远"，DDC 关心"走的方向对不对"——一个贴着路线但原地转圈的轨迹 EP 可能不低，DDC 却会把它打爆，专门防"为安全原地转圈"的刷分漏洞。

**② TLC（红绿灯合规 / Traffic Light Compliance）——v2 新增**
- **怎么算**：在轨迹经过的每个**受红绿灯控制的交叉口/车道**，把自车状态与该车道在仿真时刻的**红绿灯相位**对齐：若灯为**红/黄（需停）**而自车仍进入停止线内或冲突区 → TLC=0；若灯为**绿**且自车正常通过，或在红灯前于停止线后合规停车 → TLC=1。它直接用 nuPlan 标注里的交通灯状态，是 v1 完全没有的"规则层"检查——v1 只管物理安全，不管你闯不闯红灯。

**③ EP 的升级：Multi-Path Progress（多路径进度，MPP）——v2 核心改动**
- **v1 的坑**：EP 只拿**一条**人类专家轨迹当参考。但同一个路口，人类可能直行也可能右转，模型选了"另一条同样合理"的车道就会被错罚——这不公平，也让 EP 对单条专家轨迹过拟合。
- **v2 怎么改**：用**多条候选参考路径**（同一场景里所有"合理可达"的路线，如不同车道、不同转向）分别计算自车沿每条路的进度，再取**最优（最大）**那条当作 EP 的分母参照。这样只要你的轨迹沿"任何一条合理路径"在好好前进，EP 就高——既连续又公平，不会因为走了和专家不一样的合法车道就被扣分。

**④ LK（车道保持 / Lane Keeping）——v2 新增**
- **怎么算**：逐帧检查自车**横向偏移**是否稳定落在当前车道中心带内（一个小阈值窗口）。全程居中 → LK=1；频繁压线、蛇形、骑线 → LK=0。**关键例外**：在交叉口 / 无明确车道线的区域，LK 会被**主动关闭**（因为那本就该变道/汇入，不该用车道中心卡你）。所以 LK 考的是"该居中时居中"，不是"全程不许动"。

**⑤ HC（历史舒适度 / History Comfort）——v2 新增，时序一致性的第一块拼图**
- **怎么算**：v1 只在那**孤立的 4 秒预测片段**上评舒适度，等于默认"第 0 秒车是凭空开始动的"，会漏掉"从真实驾驶切进预测"那一刻的顿挫。v2 的做法是：把预测轨迹**前面接上 1.5 秒的人类驾驶历史**（你真实开到的最后 1.5s），拼成一条 **5.5 秒的连续时间线**，再在这条完整线上算加速度 / jerk 是否超标。这样"预测起点猛地一脚油门/一脚刹车"这种边界跳变也会被抓住。

**⑥ EC（扩展舒适度 / Extended Comfort）——v2 新增，时序一致性的第二块拼图**
- **怎么算**：不只看绝对值，而是**逐相邻帧**比较轨迹的动态状态（速度、加速度、jerk）。如果某一帧到下一帧之间出现**加速度 / jerk 的突跳**（比如速度曲线不光滑、帧间瞬移感），就判 EC=0。它本质在考"轨迹在相邻时刻是不是接得上、动得平不顺"——这就是"连续时序一致性"在舒适度上的直接落地。

#### 连续时序一致性：v2 怎么管住"轨迹在时间上得连续自洽"

你领导提到的"连续时序一致性"，指的就是 v2 相对 v1 在**时间维度**上补的那一刀。展开讲：

- **v1 的问题**：只在"一段孤立的 4 秒预测"上、按**离散帧**抽样打分。两段几乎一样的轨迹，只要在某次采样帧上差一点，分数就可能离散跳变；而且它不关心"预测起点和真实驾驶怎么接"、"相邻帧动得顺不顺"——天然漏掉时序上的毛刺。
- **v2 的解法 = HC + EC 双保险 + 连续评估**：
  1. **HC（历史舒适度）**把"过去 1.5s 真驾 → 未来 4s 预测"接成一条连续时间线，保证**起点的衔接是顺的**，不会在切换瞬间抖一下；
  2. **EC（扩展舒适度）**逐相邻帧比较动态状态，保证**预测内部的每一小步也是接得上的**，加速度/jerk 不能突跳；
  3. 更一般地说，v2 的 TTC、EP（MPP）、进度等子指标都是**沿整条时间轴细粒度连续评估**的——轨迹微小变化只引起分数微小变化，不再有 v1 那种"采样帧一切换就跳档"的离散性。
- **它实际抓住什么**：有些规划器"开头猛、中间稳"或"帧间微抖一下"，v1 测不出；v2 用 HC 抓起点顿挫、用 EC 抓帧间跳变，把这些"时间上不自洽"的轨迹打下去。所以"连续时序一致性"不是某个单独叫这名字的子指标，而是 v2 通过 **HC + EC + 连续化评分**共同实现的一条设计原则：**好轨迹不仅要空间上安全合规，还要在时间上平滑、衔接、自洽。**

**为什么 v2 更难**：DDC / TLC / LK / HC / EC 这些新增项把"大致合规"升级成"精细合规 + 时序自洽"（不光不越野，还要居中车道；不光不撞，还要朝正确方向、按红绿灯走；不光动得安全，还要动得平滑衔接），所以同一方法在 navtest（PDMS）和 navtest v2（EPDMS）上分数会差一截——这也解释了为什么榜单上 EPDMS 普遍比 PDMS 低几个点。
