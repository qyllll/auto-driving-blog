...
> **一句话总结**：PDMS 的 5 个子指标里，只有 EP 是连续值，其余 4 个（NC/DAC/TTC/C）本质都是"达标 1 / 不达标 0"的开关；其中 NC 和 DAC 是**乘性短板**（任一为 0 整场景归零），TTC/EP/C 是**加权鼓励**（越满越好）。撞车这件事，计分逻辑是"逐帧检测结果→按责任定 NC（0/0.5/1）、按越界定 DAC、按最小安全时距定 TTC、按实际位移定 EP"，而不是"撞了就停、给 0"。
