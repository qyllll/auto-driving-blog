...
PDMS（Predictive Driver Model Score）由**乘性惩罚项**（违规直接打折）和**加权平均项**（非违规指标加权）两部分组成。官方定义为：
...
图像 (多视角环视) ──► Image Encoder (DINOv2 ViT-S, 冻结)
                                   │
                                   ▼
                        Proposal Generator (轻量 Transformer)
                                   │  生成 N 条候选轨迹
                                   ▼
...
图像 + 指令 ──► VLM Backbone
       ...
       ┌──────────┼──────────┐
       ▼          ▼          ▼
   感知 (Perceive) 预测 (Predict) 规划 (Plan)
   "附近有什么"  "它们会怎么动"  "我该怎么做"
       │          │          │
       └──── P³-CoT 结构化标签 ────┘
                  │
...
```
随机噪声轨迹 x_T ──► Denoising Network (条件: 场景特征)
       │                    每步从 anchor 高斯 而非 标准高斯 采样
       ▼
   x_{T-1} ... x_0 (候选轨迹, 多模态)
       │
       ▼
...
```
噪声轨迹 ──► Flow Matching 向量场网络 (条件: 场景)
                  │
                  ├─ 无条件分支 (纯轨迹流)
...
看懂分数前，先得看懂分母。NAVSIM 的分数不是"轨迹像不像人驾"，而是把一条轨迹丢进**非反应式仿真**里跑，再算多个子指标加权。理解子指标，才能理解各家方法为什么在某项上高、某项上低。

**PDMS（Predictive Driver Model Score，v1）** 由 5 个子项乘性组合：

| 子指标 | 含义 | 考察能力 |
|:------:|------|---------|
| **no**（no at-fault collision） | 无责碰撞率 | 避障、安全距离 |
| **dj**（drivable area compliance） | 可行驶区域合规性 | 不骑马路沿、不越野 |
| **tc**（traffic compliance） | 交通规则合规性 | 红绿灯、让行、限速 |
| **com**（comfort） | 舒适度 | 加速度/ jerk 不突变 |
| **ep**（ego progress） | 自车前进进度 | 不原地犹豫、有效通行 |

PDMS = 上述子项的加权几何平均（更接近"木桶短板"逻辑，某项崩了总分直接掉）。

**EPDMS（Extended PDMS，v2）** 在 PDMS 基础上扩展，主要新增/强化了：

- **lane keeping（车道保持）**：是否稳定居中在车道内，而非只在可行驶区域大致合规；
- **traffic light compliance（红绿灯合规）**：v2 更严格地考红绿灯状态；
- **direction compliance（方向合规）**：是否朝目标方向行进（防止"为了安全原地转圈"刷 no 分）；
- **extended comfort（扩展舒适度）**：更细的动力学平滑性约束。

**navhard 的两阶段**。EPDMS 在 navhard 上要算两轮：
- **Stage 1**：在原始 val 场景上评；
- **Stage 2**：用 3DGS 把场景做"观测偏移"（摄像头视角、物体位置做物理合理的微扰）生成新场景，再评一次；
- 最终 navhard 分数 = 两阶段综合。Stage 2 专门暴露"模型是否真的理解场景动力学"——纯靠记忆分布内模式的模型在这里会大幅掉分。
