...
**基础数据集划分（OpenScene）**。OpenScene 自身提供 `trainval`（训练+验证，大）、`test`（测试，小）、`mini`（演示）三个标准划分。NAVSIM 在这之上叠加"困难场景过滤"，得到研究论文最常用的三个拆分：

| 拆分 | 基础 | 用途 | 大致规模 |
|:----:|:----:|:----:|:--------:|
| **navtrain** | OpenScene `trainval` | 训练集 | ~9000+ 场景（论文报告约 9000+ 帧用于训练） |
| **navtest** | OpenScene `test` | v1 评测集（PDMS） | **2146 个场景**（这就是排行榜打分所跑的 val） |
| **navhard_two_stage** | OpenScene `test` | v2 难例评测集（EPDMS，两阶段） | 450 真实 + 5462 合成 |

你提到的"9000 train / 1000 calib / 2146 val"正对应这个结构：`navtrain` 取自 `trainval` 过滤出的训练帧（约 9000+），`trainval` 里还留了一部分做校准/验证（calib），而 `navtest` 的 **2146 个场景**就是论文和官方榜用来出 PDMS 分数的验证集。一句话总结：**大家刷的 NAVSIM v1 navtest 分数，就是模型在 OpenScene-test 上过滤出的 2146 个挑战性场景上跑非反应式仿真算出来的 PDMS**。

**关键约束**：`navtest` / `navhard` 这些测试拆分**禁止用于训练**（防止刷榜），可用其他公开数据或预训练权重，但必须在技术报告里声明数据使用。

### NAVTEST 是什么

**NAVTEST（通常写作 navtest）就是 NAVSIM v1 的标准测试拆分**——基于 OpenScene 的 `test` 划分、经困难场景过滤后得到的 2146 个场景。它的评测指标是 **PDMS**（v1）。榜单上所有"PDMS 94.5 / 93.7 / 92.1..."之类的数字，都是模型对这 2146 个场景各输出一条轨迹，丢进非反应式仿真算 PDMS 后取平均得到的。所以当你看到论文说"在 NAVSIM 上 PDMS 94.5"，等价于"在 navtest 的 2146 个场景上 PDMS 均值 94.5"。
...
最终榜单上的 PDMS = 对所有评测场景（navtest 的 2146 个）的 PDMS 取**均值**：
