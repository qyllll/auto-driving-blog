...
├─────────────────────────────────────────────────────────┤
│                                                         │
│  ① 从图像中采样 context block（保留区域）                   │
│  ② 采样多个 target block（遮挡区域，需大尺度）              │
│                                                         │
│  ③ context encoder (ViT)                                 │
│     → context representation                             │
│                                                         │
│  ④ predictor (轻量 Transformer)                          │
│     → 预测 target representations                        │
│                                                         │
│  ⑤ target encoder（动量更新）                              │
│     → 真实 target representations                        │
│                                                         │
│  ⑥ L2 损失：||pred - target||² （仅在隐空间）              │
│                                                         │
│  关键：从不预测像素，只预测隐空间表征                         │
└─────────────────────────────────────────────────────────┘
```
