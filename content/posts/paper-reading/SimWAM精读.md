...
图 4 里每个场景给两组对比：左侧是原始相机视角叠加轨迹（Concat View），中间是 Ours-IL，右侧是 Ours-RL。**RL 后的 Action Expert 在交叉路口和狭窄道路中更"果断"**：沿目标方向推进更充分、完成更多机动动作，同时轨迹仍留在可行驶区域内、与周边车辆保持安全距离。模仿版则更保守，只往前挪一小段就停住——这正是"只能模仿专家"与"会自己优化驾驶质量"的直观差异。
