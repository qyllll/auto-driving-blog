...
- Input Tokenizers (three streams)
    - VQ-GAN Image Token
    - Text Tokenizer (SPM)
    - Quantized Action Token (256 bin/dim)
- three streams -> Shared Vocabulary LLM Core (Transformer Decoder Only)
- Shared Vocabulary LLM Core -> two Heads
    - Action Head (Policy) -> outputs a_t
    - Image Head (World) -> outputs o_{t+1}
- Action Attention Mask: a_t cannot attend to a_{<t} -> error propagation blocked
