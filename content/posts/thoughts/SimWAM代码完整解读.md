...
## 前置知识：世界模型（WM）→ 世界动作模型（WAM）的演进

在读 SimWAM 代码之前，先搞清楚它在大图中的位置。这一节从最基础的"世界模型"讲起，一步步讲到"世界动作模型"。

### 什么是世界模型（World Model）？

世界模型的核心思想用一句话概括：

> **让 AI 在"脑子里"想象未来会怎样——看到当前画面后，预测下一秒世界会变成什么样。**

就像人类开车时，你不需要真的冲到前面才能知道"前面那辆卡车会不会挡路"——你在脑子里"模拟"一下就知道了。

#### 世界模型的经典架构（RSSM / Dreamer 系列）

最经典的世界模型是 Dreamer 系列（DreamerV1→V2→V3），核心是 **RSSM（Recurrent State-Space Model）**：
...
**训练方式**：在真实数据上学习"给定当前观测和动作，预测下一帧"。loss = 预测帧 vs 真实帧的差异。

**用途**：训练完之后，可以在"脑子里"模拟——不用真的开车上路，就能想象"如果我往左转会怎样、往右转会怎样"，然后选最好的那个。

#### 驾驶世界模型的三个流派

| 流派 | 代表工作 | 预测什么 | 优点 | 缺点 |
|------|---------|---------|------|------|
| **像素空间** | DriveDreamer, GAIA-1, Vista | 直接预测未来视频帧 | 可视化直观，可用于数据增强 | 计算量巨大，难以实时 |
| **潜在空间** | World4Drive, DriveFuture, DLWM | 预测潜在特征（不生成图片） | 轻量快速，适合在线推理 | 不可可视化，调试困难 |
| **占用空间** | OccWorld, NIFF | 预测未来占用栅格 | 直接产出3D结构信息 | 需要占用标注，标注成本高 |

但所有这些都有一个共同问题：**世界模型只管"预测未来"，不管"怎么开车"**。它能告诉你"前面会堵车"，但不会告诉你"该不该变道"。

### 从世界模型到世界动作模型（WAM）：为什么要加"动作"？

传统世界模型的局限：

```
世界模型：  观测 → 预测未来帧 → ??? → 轨迹（还需要额外的规划器）
                       ↑
              只管预测，不管决策
```

WAM 的核心改进：**把"预测未来"和"生成动作"联合训练**，让两者互相帮助。

```
WAM：  观测 + 动作 → 同时预测未来帧 + 生成轨迹
         ↑                    ↑
    动作影响未来        未来反馈指导动作
```
...
1. **视频预测提供运动先觉**：模型在学习"这辆车接下来会怎么动"时，自然学到了"物体的运动规律"——这个知识对规划直接有用
2. **动作预测提供目标导向**：加入动作条件后，视频预测不再是"漫无目的的想象"，而是"如果我这么做，世界会怎样"——更有针对性
3. **共享表征更高效**：两个任务共享底层特征，不需要分别训练两个模型

### WAM 的架构演进：从 DriveWAM 到 SimWAM

这一节把博客里涉及的所有 WAM 论文串成一条演进线。

#### 第一代：串行 WAM（DriveWAM / DriveLaW）

```
DriveWAM / DriveLaW 的做法：
  1. 视频生成模型 → 逐帧预测未来 3s 的视频
  2. 从预测的未来帧中提取特征
  3. 规划器基于这些特征生成轨迹

问题：推理时必须真的生成未来帧 → 延迟高（~500ms+）
```

**DriveWAM**（arXiv:2504.07abor）：最早的 WAM 之一，用视频扩散模型预测未来帧，再用规划器从未来帧中规划轨迹。

**DriveLaW**（arXiv:2504.07906）：改进版，用 LLM 作为"大脑"来协调视频预测和动作生成，加入了语言推理能力。

核心问题：**推理时必须先生成未来帧，再用未来帧规划——串行执行，延迟高**。

#### 第二代：并行 WAM（Metis / BrainWAM）

```
Metis / BrainWAM 的做法：
  1. 视频专家和动作专家并行运行（Mixture-of-Transformers）
  2. 用注意力掩码隔离信息流
  3. 推理时可以只跑动作专家
...
**Metis**（arXiv:2606.15869）：用非对称注意力掩码解耦视频生成和动作预测，两个专家共享底层但输出独立。

**BrainWAM**（arXiv:2608.12854）：受大脑结构启发，用 Tri-MoT（三个专家：视觉/语言/动作）+ CAB（胼胝体动作桥）+ CIF（小脑意图融合）来协调多模态信息。

#### 第三代：极简 WAM（SimWAM）

```
SimWAM 的做法：
  1. 直接用预训练视频模型（Wan2.2-5B）当 Video Expert
  2. 一个轻量 ActionDiT 当 Action Expert
  3. 用 Isolated Attention Mask 联合训练
  4. 推理时直接扔掉 Video Expert，只跑 ActionDiT

核心创新：不重新训练视频模型，直接用现成的 → 极简
```

**SimWAM**（arXiv:2608.07468）：本文的主角。核心洞察是"视频模型的运动先觉不需要重新学，直接用预训练的就行"——用 Isolated Mask 让信息流动，推理时甩掉包袱。

### 博客里所有 WAM 论文对比

| 论文 | 视频模型 | 动作模型 | 训练方式 | 推理需要生成未来帧？ | NAVSIM PDMS | 核心创新 |
|------|---------|---------|---------|-------------------|------------|---------|
| **DriveWAM** | 自训视频扩散 | 独立规划器 | 串行两阶段 | **是** | ~88 | 最早的 WAM 范式 |
| **DriveLaW** | 自训视频模型 | LLM 规划器 | 串行 + 语言引导 | **是** | ~89 | 加入语言推理 |
| **Metis** | 自训视频 DiT | 动作 DiT | MoT 并行 | 否（可选） | ~90 | 非对称注意力掩码 |
| **BrainWAM** | 自训视频编码器 | Tri-MoT 动作专家 | 三路 MoT 联合 | 否 | ~91 | 大脑结构启发（CAB + CIF） |
| **DriveVLA-W0** | 自训视频模型 | VLA + MoE 动作头 | 视频提供稠密监督 | 否 | ~91 | 视频模型放大数据缩放律 |
| **Percept-WAM** | 视频模型 + 感知增强 | VLA 动作头 | 2D/3D 感知注入 | 否 | ~92 | World-PV/BEV token |
| **ReWorld** | 基于 DriveLaW | DriveLaW 架构 | 表征学习 | 是 | ~90 | 未来预测监督 + hard negative |
| **SimWAM** | **Wan2.2-5B（预训练）** | **ActionDiT** | **联合 Flow Matching** | **否** | **91.5** | **极简：直接用预训练视频模型** |

### WAM 的核心权衡：推理延迟 vs 轨迹质量

所有 WAM 都面临一个根本权衡：

```
生成未来帧越多 → 运动先觉越丰富 → 轨迹质量越高
                → 但推理延迟越大 → 实时性越差
```
...
| 方案 | 推理时做什么 | 延迟 | 轨迹质量 |
|------|------------|------|---------|
| **无世界模型**（UniAD/VAD） | 直接从观测映射到轨迹 | **极低** | 中等（缺时序建模） |
| **串行 WAM**（DriveWAM） | 先生成未来帧，再规划 | **高** | 高 |
| **并行 WAM**（Metis/BrainWAM） | 两个专家并行，但都需要运行 | 中等 | 高 |
| **SimWAM** | **只跑 ActionDiT** | **极低** | 高（靠训练时借力） |

SimWAM 的核心贡献就是找到了一个**帕累托最优解**：通过 Isolated Mask 在训练时把运动先觉"蒸馏"到 ActionDiT 里，推理时完全不需要 Video Expert——既保留了世界模型的运动理解能力，又去掉了推理时的延迟包袱。

### 一句话总结 WAM 的演进

> **世界模型（WM）= 教 AI "想象未来"；世界动作模型（WAM）= 教 AI "边想象边决策"；SimWAM = "训练时想象、推理时只决策"——既借了世界模型的力，又不背它的包袱。**
