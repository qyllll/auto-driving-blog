...
**① 先认识主角：噪声 $x_0$、图片 $x_1$，以及"在它们中间"的 $x_t$**

- $x_0$：`t=0` 那一端，一张全是随机雪花点的图（像素值全是随机数）。
- $x_1$：`t=1` 那一端，我们最终想要的那张图（比如 "a red cat"）。
- 中间的 $x_t$：噪声朝图片走了一半时的状态。可以把 $t$ 理解成**进度条**（0=还没走，1=走完了）。

这两个主角之间的"路"怎么定义？——在两点之间拉一条直线（这叫**线性插值**，就是把两点中间的空白点按比例填上）：

$$
x_t = (1-t)\,x_0 + t\,x_1, \qquad t\in[0,1]
$$

逐项翻译成大白话：$t=0$ 时 $x_0$ 是纯噪声；$t=1$ 时是纯图片；$t=0.5$ 时每个像素取"噪声和图片各一半的平均"。$t$ 就是你站在这条直线上的位置。
...
先别怕 $\frac{dx}{dt}$，它读作"**x 对时间的变化率**"，就是**瞬时速度**，和汽车仪表盘上那一刻的时速是一个意思。对直线来说，速度 = 终点 − 起点 = "从噪声指向图片的那个方向"。**关键点：沿这条直线，速度永远是同一个常数**——这就是为什么图里每个小方框都写着 `v·Δt`，因为每步都朝同一个方向、用同一个速度。
...
**③ 推理：生成图片 = 解方程，这里必须讲清楚"积分"和"Euler"**
...
**那 Euler 又是什么？** 积分不能一笔算完（因为速度本身依赖位置，循环依赖），所以只能**近似**。最朴素的做法就是欧拉法（以数学家 Euler 命名），思路一句话：**"假设每个小时间段 $\Delta t$ 内速度不变，用这段开始那一刻的速度走完这一段。"** 走完再重新看速度、再走。公式就是图里那个：
...
- 速度变化越快，$\Delta t$ 切得越小越准。图里把 $t:0\to1$ 切成 **10 段**（`num_inference_steps=10`），每段 $\Delta t=0.1$，10 个小方框就是 10 次 Euler 步进。
...
GRPO 是策略梯度方法，核心诉求是：**"某条路走出来的概率是多少"，然后用奖励把它推高或压低**：

$$
\nabla_\theta \mathcal{L} = -\,\mathbb{E}_\tau\Big[\, A(\tau)\; \nabla_\theta \log\pi_\theta(\tau) \Big]
$$

翻译：对每条生成的轨迹 $\tau$，用"它的奖励优势 $A(\tau)$"去乘"它概率的对数导数"，再加总平均。效果就是——**好事（A>0）的轨迹概率调大，坏事调小**。

但纯 ODE 有**两个致命问题**，两个都不满足上面公式的要求：

1. **没有随机性 = 没有可调的概率。** ODE 是"复印机"：同样的噪声进去，永远出来同一张图。就像一位从不失误、每次都做同一个动作的运动员——他没有任何"发挥好坏的变化"，教练根本无从指导。没有随机性，"概率"要么是 1、要么是 0，梯度算不出来。
2. **没有随机性 = 无法探索。** 让图片变好，必须靠"多试试不同的中间路线，看哪条 reward 高"。纯 ODE 每次路线都一样，连试的机会都没有。
...
**⑤ 离散 SDE 一步（Euler–Maruyama）+ log_prob**

"Euler–Maruyama"就是 **Euler + 噪声**：每走一步，除了按 Euler 的规则前进，还额外叠一个高斯随机数。
...
（由当前噪声水平 $\sigma$ 和预设强度 `noise_level` 算出这一步的"抖动幅度"。）
...
别被长式子吓到：它本质就是"**带了一点修正的 Euler 步**"——$x_t$ 往前走一点、$v_\theta$ 加上一点，多出来的 $(1+\cdots)$ 系数是 SDE 理论里的"漂移修正"，保证加噪后整体方向不偏。**你只需要记住：mean = 下一步最可能的位置。**
...
翻译：以 mean 为中心、$\sigma_{\mathrm{noise}}^2(-\Delta t)$ 为散布，按**高斯分布（钟形曲线）**抽一个点——**大概率落在 mean 附近，偶尔偏出去一点**。这一步就是"随机策略"真正落地的地方：每次采样都得到一条略有不同的轨迹。

既然 $x_{t+1}$ 是从一个高斯里抽出来的，就能算出"刚才正好抽到它"的概率（对数形式，公式看着长，逐项读）：
...
- 第一项 $-\frac{\|x_{t+1}-\text{mean}\|^2}{2\sigma^2(-\Delta t)}$：**离 mean 越远，越不像是随机抽出来的**（偏离越离谱，惩罚越重）。
- 第二项 $-\log(\sigma_{\mathrm{noise}}\sqrt{-\Delta t})$：抖动越大，单个点被抽中的概率越小（分布摊开了）。
- 第三项 $-\log\sqrt{2\pi}$：高斯分布的"归一化常数"，保证所有概率加起来是 1，跟数据无关。
...
- $p(x_k\mid x_{k-1},c)$：已知上一步，这一步会走到哪的概率——正是附 A 那个**高斯核**。
