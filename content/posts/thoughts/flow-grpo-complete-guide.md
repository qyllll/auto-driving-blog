...
### 0.4 DiT、Transformer、Flow Matching 到底什么关系？（常见困惑澄清）

很多人读到这里会卡住：**DiT 输出速度场，Flow Matching 也说学习速度场——到底谁是输出、谁是训练目标？** 这一节把逻辑捋清楚。

#### 先搞清楚一件事：DiT 就是那个神经网络

**DiT（Diffusion Transformer）= 把 Transformer 用在扩散模型里的神经网络。** 它和普通 Transformer 几乎没有结构区别：

```
普通 Transformer（GPT）：  输入文字 token → Self-Attention × N层 → 输出下一个 token 的概率
DiT（FLUX）：             输入噪声 latent + 时间步 t → Self-Attention × N层 → 输出速度向量
```

Transformer 本身只是一个"输入向量 → 输出向量"的函数拟合器。**它输出什么，完全取决于你怎么训练它。**

#### 为什么 DiT 的输出是速度场？——因为训练就这么教的

Flow Matching 的训练目标是：

$$\mathcal{L} = \mathbb{E}\Big[\big\| v_\theta(x_t, t) - (x_1 - x_0) \big\|^2\Big]$$

翻译成人话：
1. 准备一对数据：$x_0$ = 纯噪声，$x_1$ = 真实图片/轨迹
2. 随机采一个时间步 $t$，插值得到中间状态 $x_t = (1-t) \cdot x_0 + t \cdot x_1$
3. 把 $(x_t, t)$ 喂给 DiT，得到输出 $v_\theta(x_t, t)$
4. **正确答案**是 $x_1 - x_0$（从噪声指向真实数据的方向 = 真实速度）
5. 算 loss = $\|v_\theta - (x_1 - x_0)\|^2$，反向传播更新 DiT 的权重

训练完之后，DiT 的输出**自然就是速度场**——因为它是被这么训练的。

**类比**：一个学生（DiT）被反复训练做"1+1=?"（输入数字，输出和）。训练完之后你问他"2+3=?"，他输出"5"。不是因为他天生就会加法，而是因为**训练教了他"输入数字 → 输出和"这个映射**。DiT 也一样：训练教了它"输入带噪状态+时间步 → 输出速度"。
...
训练时（Flow Matching）：
  1. 准备数据对：(x₀=噪声, x₁=真实图片)
  2. 随机采时间步 t，插值得到 xₜ
  3. DiT 输入 (xₜ, t)，输出 v_θ(xₜ, t)
  4. 正确答案：x₁ - x₀（真实速度）
  5. Loss = ||v_θ - (x₁-x₀)||²，更新 DiT 参数
  → 反复训练后，DiT 学会了"在任何中间状态，该往哪个方向走"

推理时（生成图片/轨迹）：
  1. 从随机噪声 x₀ 开始
  2. DiT 输入 (x₀, t=0)，输出 v_θ → "第一步该往哪走"
  3. 走一步：x₁ = x₀ + v_θ · Δt
  4. DiT 输入 (x₁, t=0.1)，输出 v_θ → "第二步该往哪走"
  5. 再走一步：x₂ = x₁ + v_θ · Δt
  6. 重复 10 步，最终得到清晰的图片/轨迹
```

**DiT 在推理时的角色就是一个"导航仪"——你告诉它"我现在在哪（$x_t$）、走了几步了（$t$）"，它告诉你"下一步该往哪走（$v_\theta$）"。**

#### Flow Matching 比扩散模型好在哪里？——详细对比

上面说的 DiT 是"骨架"，Flow Matching 和扩散模型（DDPM）是两种不同的"训练方法"。它们训练出来的都是"去噪/生成模型"，但实现路径完全不同。
...
```
真实数据 x₁ → 逐步加噪 → 纯噪声 xₜ（前向过程，固定公式）
纯噪声 xₜ → 逐步去噪 → 生成数据 x̂₁（反向过程，网络学习）
```

网络学的是"每一步去掉多少噪声"（噪声预测 $\epsilon_\theta$），走 20~50 步才能从噪声变成清晰图片。

**Flow Matching**：直路——从噪声直线走到数据
...
网络学的是"在这条直线上，每一步该往哪个方向走"（速度场 $v_\theta$），走 10 步就够。

##### 五个维度逐项对比

| 维度 | 扩散模型（DDPM） | Flow Matching |
|------|------|------|
| **前向过程** | 逐步加噪（马尔可夫链，T步） | **直线插值**（一步到位） |
| **网络学什么** | 预测噪声 $\epsilon_\theta(x_t, t)$ | 预测速度场 $v_\theta(x_t, t)$ |
| **推理步数** | 20~50 步 | **5~10 步** |
| **训练目标** | $\|\epsilon_\theta - \epsilon\|^2$（猜对噪声） | $\|v_\theta - (x_1-x_0)\|^2$（猜对速度） |
| **数学基础** | 随机微分方程（SDE） | 常微分方程（ODE） |

##### 为什么 Flow Matching 更快？

扩散模型的前向过程是"逐步加噪"——从 $t=0$ 到 $t=T$，每步加一点高斯噪声，形成一条**随机游走**的路径。反向去噪时，网络需要沿着这条随机路径**一步一步退回去**，步数少了精度不够。

Flow Matching 的前向过程是**直线插值**——从噪声到数据拉一条直线。因为路径是直的，速度场几乎恒定（等于 $x_1 - x_0$），所以网络学起来更容易、推理时步数可以更少。

直觉类比：
- 扩散模型 = 蒙着眼走迷宫，每一步只能摸到附近一小块，需要慢慢试
- Flow Matching = 给你一张地图上的直线，直接朝着目的地走

##### 为什么 Flow Matching 更适合 GRPO？

GRPO 需要**反复采样**（每个场景采 8~24 条轨迹/图片），然后打分、更新。采样次数越多，推理速度越关键。

| | 扩散模型 | Flow Matching |
|---|---|---|
| 单次采样步数 | 50 步 | 10 步 |
| 采 24 张图的总步数 | 24 × 50 = 1200 步 | 24 × 10 = 240 步 |
| 采样速度 | 慢（5×） | **快（5×）** |

这就是为什么 Flow-GRPO 选择 Flow Matching 而不是 DDPM——**采样效率差 5 倍，RL 训练就快 5 倍**。

##### 训练目标的直觉对比

**扩散模型学"噪声长什么样"**：

$$\mathcal{L}_{\text{DDPM}} = \mathbb{E}\Big[\big\| \epsilon_\theta(x_t, t) - \epsilon \big\|^2\Big]$$

网络需要从带噪图片中"猜"出叠加的噪声是什么。猜对了，减掉噪声就得到清晰图片。

**Flow Matching 学"该往哪走"**：

$$\mathcal{L}_{\text{FM}} = \mathbb{E}\Big[\big\| v_\theta(x_t, t) - (x_1 - x_0) \big\|^2\Big]$$

网络需要判断"从当前点出发，往真实数据方向走多远"。走对了，一步步走就到达清晰图片。

两种方式最终都能生成好图片，但 Flow Matching 的"速度场"概念更直观、路径更短、步数更少。

##### 一句话总结

> **扩散模型是"弯路去噪"（50步），Flow Matching 是"直线导航"（10步）。结构一样（都是 DiT），训练目标不同（噪声 vs 速度），但 Flow Matching 快 5 倍——这就是它成为 2025 年主流选择的原因。**

#### 那 GRPO 改的是什么？

GRPO 不改 Flow Matching 的训练方式，而是**在训练好之后，用 RL 微调 DiT 的参数，让它输出的速度场方向更符合奖励函数**。

```
SFT 阶段：
  DiT 学会了"从噪声走到图片/轨迹的速度场"
  → 生成的图片/轨迹是"合理的"（像训练数据）

GRPO 阶段：
  用 LoRA 微调 DiT 的参数
  → 速度场方向被微调
  → 生成的图片/轨迹变成"高奖励的"（符合 reward 标准）
```

打个比方：
- SFT = 教一个学生"会做题"（学会基本能力）
- GRPO = 教这个学生"做对的题多做，做错的题少做"（用奖励信号微调方向）
- LoRA = 给学生戴一个"微调手环"，轻轻调整做题习惯，不换脑子

#### 三者的关系总结

| 概念 | 是什么 | 类比 |
|------|------|------|
| **Transformer** | 一种神经网络架构（Attention 机制） | 一个"大脑"的结构 |
| **DiT** | 把 Transformer 用在扩散模型里 | 这个大脑被训练来"看噪图、出速度" |
| **Flow Matching** | 训练 DiT 的方法（loss 函数） | 教这个大脑"速度场应该长什么样" |
| **速度场** | DiT 的输出（每步该往哪走） | 大脑给出的"导航指令" |
| **GRPO** | 训练好之后的 RL 微调 | 用奖励信号微调导航方向 |
| **LoRA** | 微调的参数高效手段 | 只调一小部分参数，不换脑子 |
